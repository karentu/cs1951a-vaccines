...
table = """CREATE TABLE school_dem(id INTEGER PRIMARY KEY AUTOINCREMENT, \
    county TEXT, school TEXT, total_enrollment INTEGER, american_indian_alaskan_native INTEGER, \
    asian INTEGER, asian_pacific_islander INTEGER, pacific_islander INTEGER, \
    black INTEGER, hispanic INTEGER, white INTEGER, two_or_more_race INTEGER, \
        ...
        with open(getfile(k,n),'r') as fin: # `with` statement available in 2.5+
            ...
            for row in to_db:
                ...
                insrt_str = """INSERT INTO school_dem(county, school, total_enrollment, \
                    american_indian_alaskan_native, asian, asian_pacific_islander, pacific_islander, \
                    black, hispanic, white, two_or_more_race, male, female, num_free_reduced_meals, \
                    district_number, city, zip, school_code, year) \
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"""
